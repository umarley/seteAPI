...
use Sete\V1\Rest\Ordens_Servico;
use Sete\V1\API;

class Ordens_ServicoResource extends API
{
    /**
     * Create a resource
     *
    ...
    private function processarRequestPOST($codigoCidade, $arData) {
        ...
        if ($usuarioPodeAcessarMunicipio) {
            ...
            $this->processarInsertOrdensServico($arData);
        } else {
            $this->populaResposta(403, ['result' => false, 'messages' => 'Usuário sem permissão para acessar o municipio selecionado.'], false);
        }
    }

    private function processarInsertOrdensServico($arData) {
        $modelOrdensServico = new Ordens_ServicoModel();
        $boValidate = $modelOrdensServico->validarInsert($arData);
        if ($boValidate['result']) {
            $arResult = $modelOrdensServico->prepareInsert($arData);
